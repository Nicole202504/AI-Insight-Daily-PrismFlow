      ...
      <ReactMarkdown 
        ...
        components={{
          h1: ({ node, children, ...props }) => {
            ...
            return <h1 {...props} id={slugifyHeading(text)}>{children}</h1>;
          },
          h2: ({ node, children, ...props }) => {
            const text = String(children || '');
            return <h2 {...props} id={slugifyHeading(text)}>{children}</h2>;
          },
          h3: ({ node, children, ...props }) => {
            const text = String(children || '');
            return <h3 {...props} id={slugifyHeading(text)}>{children}</h3>;
          },
          a: ({ node, ...props }) => (
            <a {...props} target="_blank" rel="noopener noreferrer" />
          ),
          img: ({ node, ...props }) => (
            <img {...props} src={proxyImageUrl(props.src || '', imageProxy)} alt={props.alt || ''} className="my-6 rounded-lg border border-slate-200 dark:border-white/10 shadow-sm" />
          ),
          video: ({ node, ...props }) => (
            <video 
              {...props} 
